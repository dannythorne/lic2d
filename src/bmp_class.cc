...
void bmp_class::read_header()
{
  char *inbuf4;
  int inbuf;

  char *inbuf2;
  short int inbuf_short;

  fin >> type[0];
  fin >> type[1];

  inbuf4 = (char*)&inbuf;
  fin.read( inbuf4, 4); size = ENDIAN4(inbuf);

  fin.read( inbuf4, 4); rsvd = ENDIAN4(inbuf);
  fin.read( inbuf4, 4); dptr = ENDIAN4(inbuf);
  fin.read( inbuf4, 4); forty  = ENDIAN4(inbuf);
  fin.read( inbuf4, 4); width  = ENDIAN4(inbuf);
  fin.read( inbuf4, 4); height = ENDIAN4(inbuf);

  inbuf2 = (char*)&inbuf_short;
  fin.read( inbuf2, 2); planes = ENDIAN2(inbuf_short);
  fin.read( inbuf2, 2); depth  = ENDIAN2(inbuf_short);

  inbuf4 = (char*)&inbuf;
  fin.read( inbuf4, 4); zeros[0] = ENDIAN4(inbuf);
  fin.read( inbuf4, 4); zeros[1] = ENDIAN4(inbuf);
  fin.read( inbuf4, 4); zeros[2] = ENDIAN4(inbuf);
  fin.read( inbuf4, 4); zeros[3] = ENDIAN4(inbuf);
  fin.read( inbuf4, 4); zeros[4] = ENDIAN4(inbuf);
  fin.read( inbuf4, 4); zeros[5] = ENDIAN4(inbuf);

} /* void bmp_class::read_header() */

void bmp_class::disp_header()
{
  int n;
  ...
  cout << "zeros[1] = " << zeros[1]  << endl;
  cout << "zeros[2] = " << zeros[2]  << endl;
  cout << "zeros[3] = " << zeros[3]  << endl;
  cout << "zeros[4] = " << zeros[4]  << endl;
  cout << "zeros[5] = " << zeros[5]  << endl;

  //cout << "\"" << bmp_hdr->zeros  << "\"" << endl;

} /* void bmp_class::disp_header() */

void bmp_class::read_true_color()
{
  BING("void bmp_class::read_true_color()");
  ...
  cout << "  bytes_of_pixels_per_row = " << bytes_of_pixels_per_row() << endl;
  cout << "  bytes_of_pad = " << bytes_of_pad() << endl;
  ...
  for( j=0; j<height; j++)
  {
    ...
    for( i=0; i<width; i++)
    {
      fin >> buf[0]; b[n] = ( (int)*buf+256) % 256;
      fin >> buf[0]; g[n] = ( (int)*buf+256) % 256;
      fin >> buf[0]; r[n] = ( (int)*buf+256) % 256;
    ...
    const int arg_depth)
{
  ...
  {
    r = new int[ num_pixels()];
    g = new int[ num_pixels()];
    b = new int[ num_pixels()];
  }
  else
  {
    BING("Error.");
  ...
  {
#if 0
    r[n] = n;//(int)floor( 255.*(double)n/(double)num_pixels());
    g[n] = n;
    b[n] = n;//(int)floor( 255.*(double)( num_pixels() - n)/(double)num_pixels());
#else
    r[n] = 0;
    g[n] = 0;
    b[n] = 0;
...
void bmp_class::init_header()
{
  type[0] = 'B';
  type[1] = 'M';
  size = height * ( bytes_of_pixels_per_row() + bytes_of_pad());
  rsvd = 0;
  dptr = 54; // TODO: Generalize this.
  forty = 40;
  planes = 1;
  zeros[0] = 0;
  zeros[1] = 0;
...
void bmp_class::write()
{
  ...
  inbuf4 = (char*)&inbuf;
  inbuf = ENDIAN4( size); fout.write( inbuf4, 4);

  inbuf = ENDIAN4(rsvd); fout.write( inbuf4, 4);
  inbuf = ENDIAN4(dptr); fout.write( inbuf4, 4);
  inbuf = ENDIAN4(forty); fout.write( inbuf4, 4);
  inbuf = ENDIAN4(width); fout.write( inbuf4, 4);
  inbuf = ENDIAN4(height); fout.write( inbuf4, 4);

  inbuf2 = (char*)&inbuf_short;
  inbuf_short = ENDIAN2(planes); fout.write( inbuf2, 2);
  inbuf_short = ENDIAN2(depth); fout.write( inbuf2, 2);

  inbuf4 = (char*)&inbuf;
  inbuf = ENDIAN4(zeros[0]); fout.write( inbuf4, 4);
  inbuf = ENDIAN4(zeros[1]); fout.write( inbuf4, 4);
  inbuf = ENDIAN4(zeros[2]); fout.write( inbuf4, 4);
  inbuf = ENDIAN4(zeros[3]); fout.write( inbuf4, 4);
  inbuf = ENDIAN4(zeros[4]); fout.write( inbuf4, 4);
  inbuf = ENDIAN4(zeros[5]); fout.write( inbuf4, 4);
...
void bmp_class::flipud()
{
  int i, j;
  int m, n;
  int rtemp, gtemp, btemp;

  for( j=0; j<(int)floor((double)height/2.); j++)
  {
    for( i=0; i<width; i++)
    {
      m = j*width + i;
      n = (height-j-1)*width + i;
      //n = j*width + (width-n-1);
...
void bmp_class::scroll_left( const int num_cols)
{
  int i, j;
  int m, n;
  int rtemp, gtemp, btemp;
...
void bmp_class::scroll_down( const int num_rows)
{
  int i, j;
  int m, n;
  int rtemp, gtemp, btemp;
...
void dump_sigma_btc( lattice_ptr lattice)
{
  ...
  btc_spot = 
    ...
    :
    (((lattice->FlowDir==1)?(get_LX(lattice)):(get_LY(lattice)))-2-1);
